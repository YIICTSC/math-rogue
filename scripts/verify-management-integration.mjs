...
assert.doesNotMatch(service, /URL\.createObjectURL/);
...
assert.doesNotMatch(inviteModal, /childSafetyService\.isChild\(\)/);
...
const learnerInvitationJoin = service.slice(service.indexOf('async joinLearnerInvitation'), service.indexOf('async linkDevice'));
assert.match(learnerInvitationJoin, /displayName: input\.displayName/);
assert.match(learnerInvitationJoin, /attendanceNumber: input\.attendanceNumber/);
assert.doesNotMatch(learnerInvitationJoin, /ageBand/);
