...
4. ホスト側でリモート操作を処理した直後、必ず同期が発火する保証を強める。 **対応済み**
   - 現状は `coopBattleState` 変更に反応する effect と個別 broadcast が混在している。
   - 選択解除のように `selectionState` だけ変わるケースは特に漏れやすい。
   - ゲスト由来のカード使用、ポーション使用、ターン開始、手札選択、山札モーダル、支援カード使用の処理後に、ホストが最新 `stateRef.current.coopBattleState` を短い遅延で再送する共通予約を追加。
   - 敵選択の同期は直接 `p2pService.send` ではなく `broadcastCoopBattleState()` 経由へ寄せ、戦闘同期ペイロードの差分を減らした。
...
- 2026-07-09
  ...
  - ホストがゲストの戦闘操作を処理した直後に、最新状態の `COOP_BATTLE_SYNC` を予約送信する共通経路を追加。
  - 参加者の敵選択同期を `broadcastCoopBattleState()` 経由に統一。
