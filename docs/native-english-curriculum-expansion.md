...
- `src/nativeEnglishUnitConfig.ts`
  ...
  - Grade 1-8
  - ELA / Math は各学年4単元以上
  - Science / Social Studies / Japanese は各学年3単元以上

- `src/data/subjects/native_english_units.ts`
  - 既存90単元に明示的な問題定義を収録済み。
  - 追加54単元はすべて明示概念問題を収録済み。
  - `toProblems` 形式の52単元は全単元40問化済み。
  - `conceptUnit` 形式の92単元は概念問題として展開。
  - 現在の明示問題バリエーションは合計6058問相当。
...
- 直接作問単元: 52単元、2080問
  - 40問化済み: 52単元
  - 30問維持: 0単元
- 概念展開単元: 92単元、1326概念配置、3978問バリエーション
- フォールバック生成単元: 0単元
- 明示問題バリエーション合計: 6058問相当
- 設定済み144単元すべてが明示問題バリエーションを持つ
- UI上は `fillToFifty` により各単元50問相当まで補完

Phase 4以降では、`fillToFifty` のReview補完を減らすため、既存90単元と追加54単元を50問相当の完全手書きへ近づける。Phase 7では追加54単元すべてを42問相当の明示概念問題へ移行し、フォールバック生成単元を0にした。
