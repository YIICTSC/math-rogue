...
const Card: React.FC<CardProps> = ({ card, onClick, disabled }) => {
  ...
  const getTypeColor = (type: EnumCardType) => {
    switch (type) {
      case EnumCardType.ATTACK: return 'border-red-500 bg-red-900/90';
      case EnumCardType.SKILL: return 'border-blue-500 bg-blue-900/90';
      case EnumCardType.POWER: return 'border-yellow-500 bg-yellow-900/90';
  ...
  const renderDescription = () => {
      ...
      // Simple regex replacement to inject current stats into the description text
      // This assumes standard formatting like "6ダメージ" or "ブロック5"
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center relative z-10">
        <span className={`text-[12px] font-bold truncate w-20 drop-shadow-md ${card.upgraded ? 'text-green-400' : 'text-white'}`}>
            {card.name}{card.upgraded ? '+' : ''}
        </span>
        <div className="bg-blue-600 text-white w-5 h-5 flex items-center justify-center rounded text-[10px] border border-white font-bold shrink-0 shadow-sm">
          {card.cost}
        </div>
      </div>

      {/* Art */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-90 mt-[-10px]">
        {renderCardArt(card.type)}
      </div>

      {/* Description */}
      <div className="relative z-10 mt-auto">
        <div className="bg-black/40 p-1 rounded border border-white/10 backdrop-blur-[1px]">
            <div className="text-[9px] text-white leading-tight text-center h-12 flex items-center justify-center flex-col">
                {renderDescription()}
                {card.exhaust && <span className="text-gray-400 mt-1 font-bold text-[8px]">[廃棄]</span>}
            </div>
        </div>
