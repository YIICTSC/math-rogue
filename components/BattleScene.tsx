...
import { Enemy, Player, Card as ICard, CardType, SelectionState, Potion } from '../types';
import Card from './Card';
import { Heart, Shield, Zap, Skull, Layers, X, Target, Sword, AlertCircle, TrendingDown, Droplets, Hexagon, Gem, FlaskConical } from 'lucide-react';
...
const BattleScene: React.FC<BattleSceneProps> = ({ 
  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      {/* Top Bar: Narrative */}
      <div className="h-12 bg-black border-b-4 border-gray-700 flex items-center px-4 text-sm text-green-400 overflow-hidden whitespace-nowrap justify-between">
        <span className="truncate mr-4"><span className="animate-pulse mr-2">>></span> {narrative}</span>
      </div>

      {/* Battle Area */}
      <div className="flex-grow bg-gray-900 relative overflow-hidden flex justify-between items-center px-4 py-4 bg-[url('https://picsum.photos/800/400?grayscale&blur=2')] bg-cover bg-blend-multiply">
        ...
        <div className="flex flex-col items-center z-10 w-1/4">
            <div className="mb-2 w-48 bg-black/80 border-2 border-white p-2 text-white text-xs">
                ...
                <div className="flex flex-wrap gap-2 mb-2 border-b border-gray-700 pb-1">
                    ...
                    <div className="flex gap-1">
                        {player.relics.map(r => (
                            <div key={r.id} className="w-5 h-5 bg-gray-700 rounded-full border border-yellow-600 flex items-center justify-center relative group" title={r.description}>
                                <Gem size={10} className="text-yellow-400" />
                                <div className="absolute bottom-full mb-1 hidden group-hover:block bg-black border border-white p-1 text-[8px] w-24 z-50 whitespace-normal text-center pointer-events-none">
                                    {r.name}
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="flex gap-1 ml-auto">
                        {player.potions.map(p => (
                             <div 
                                ...
                                className={`w-5 h-5 bg-gray-800 rounded border border-white flex items-center justify-center relative group cursor-pointer hover:scale-125 transition-transform`}
                                title={p.description}
                             >
                                <FlaskConical size={12} style={{ color: p.color }} />
                                <div className="absolute bottom-full mb-1 hidden group-hover:block bg-black border border-white p-1 text-[8px] w-24 z-50 whitespace-normal text-center pointer-events-none">
                        ...
                        {[...Array(3 - player.potions.length)].map((_, i) => (
                             <div key={i} className="w-5 h-5 bg-black/50 rounded border border-gray-700 flex items-center justify-center">
                ...
                <div className="flex flex-wrap gap-1 mb-1">
                    {player.strength !== 0 && (
                        <span className={`flex items-center ${player.strength > 0 ? 'text-red-400' : 'text-gray-400'} text-[10px]`}>
                ...
                <div className="w-full h-2 bg-gray-700 rounded-full border border-gray-500 overflow-hidden">
                    <div className="h-full bg-green-500 transition-all duration-500" style={{width: `${playerHpPercent}%`}}></div>
                </div>
            </div>
            
            <div className={`w-32 h-32 relative transition-all duration-150 ease-out ${getActionClass()}`}>
                 <img 
                    src={HERO_IMAGE_DATA} 
                    alt="Hero" 
        ...
        {/* Center Info */}
        <div className="absolute top-12 left-1/2 -translate-x-1/2 text-center w-full pointer-events-none z-0">
            <div className="text-yellow-400 text-lg mb-2 animate-pulse font-bold drop-shadow-md">{turnLog}</div>
        </div>

        {/* Enemies */}
        <div className="flex justify-center items-end gap-2 w-3/4 pr-4 h-full pb-8">
            {enemies.map((enemy) => {
                ...
                return (
                    <div 
                        ...
                        className={`flex flex-col items-center z-10 transition-transform duration-200 cursor-pointer ${isSelected && !actionClass ? 'scale-110' : ''} ${!isSelected && !actionClass ? 'hover:scale-105' : ''} ${actionClass}`}
                    >
                        {/* Intent Bubble */}
                        <div className="mb-2 bg-white text-black text-[10px] p-1 rounded border-2 border-red-500 animate-bounce">
                            {enemy.nextIntent.type === 'ATTACK' && <span className="flex items-center"><Skull size={12} className="mr-1"/> {enemy.nextIntent.value}</span>}
                            {enemy.nextIntent.type === 'DEFEND' && <span className="flex items-center"><Shield size={12} className="mr-1"/> {enemy.nextIntent.value}</span>}
                            {(enemy.nextIntent.type === 'BUFF' || enemy.nextIntent.type === 'DEBUFF') && <span className="flex items-center"><Zap size={12} className="mr-1"/> !</span>}
                        </div>

                        <div className={`mb-2 w-28 bg-black/80 border-2 p-1 text-white text-[10px] transition-colors ${isSelected ? 'border-yellow-400 shadow-[0_0_10px_rgba(255,255,0,0.5)]' : 'border-red-500'}`}>
                            <div className="flex items-center justify-between mb-1">
                            ...
                            <div className="text-right mt-0.5">{enemy.currentHp} HP</div>
                        </div>

                        <div className="w-24 h-24 relative">
                            {isSelected && (
                                <div className="absolute -top-6 -left-4 text-yellow-400 animate-bounce">
                                    <Target size={24} />
                                </div>
                            )}
                            <PixelSprite seed={enemy.id} className="w-full h-full" />
                        </div>
                    </div>
                );
            })}
      ...
      <div className={`h-64 bg-gray-900 border-t-4 border-white p-4 flex flex-col relative z-20 ${selectionState.active ? 'shadow-[0_-5px_20px_rgba(255,255,0,0.3)]' : ''}`}>
        ...
        <div className="absolute -top-6 left-4 flex space-x-2">
            <div className="bg-black border-2 border-yellow-500 text-yellow-400 px-3 py-1 rounded-full flex items-center shadow-lg">
                ...
                <span className="text-lg">{player.currentEnergy}/{player.maxEnergy}</span>
            </div>
        ...
        <div className="absolute -top-6 right-4">
            <button 
                ...
                className={`bg-red-600 border-2 border-white px-6 py-2 text-sm shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all
        ...
        {/* Cards */}
        <div className="flex-grow flex items-center justify-center gap-2 px-8 overflow-x-auto pb-4 pt-2">
            {player.hand.map((card) => (
                <div key={card.id} className={`${selectionState.active ? 'cursor-pointer hover:scale-110 transition-transform' : ''}`}>
                    <Card 
                        card={card} 
                        onClick={() => {
      ...
      {showDeck && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowDeck(false)}>
            <div className="bg-gray-800 border-4 border-white w-full max-w-4xl h-[80vh] flex flex-col relative shadow-2xl" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-8 overflow-y-auto flex-grow bg-gray-900/90">
