    ...
    const handleNodeSelect = async (node: MapNode, allowRemoteCoopSelection = false) => {
        ...
        try {
            if (node.type === NodeType.COMBAT || node.type === NodeType.ELITE || node.type === NodeType.BOSS || node.type === NodeType.START) {
                ...
                const consumeEventEffect = (effectId: string): boolean => {
                    if ((p.relicCounters[effectId] || 0) <= 0) return false;
                    p.relicCounters[effectId] = 0;
                    return true;
                };
                const consumeAnyEventEffect = (effectIds: string[]): string[] => effectIds.filter(consumeEventEffect);
                const addEventLog = (message: string) => eventBattleLogs.push(trans(message, languageMode));

                const drawEventCards = consumeAnyEventEffect([
                    'EVENT_TRANSFER_STUDENT_TRADE',
                    'EVENT_NEW_SEAT_FOCUS',
                    'EVENT_LIBRARY_SILENCE',
                    'EVENT_DESK_CACHE',
                    'EVENT_FESTIVAL_POSTER',
                    'EVENT_LIBRARY_CARD',
                    'EVENT_CLASS_MANGA',
                    'EVENT_LIBRARIAN_ADVICE',
                    'EVENT_RABBIT_LUCK',
                    'EVENT_SECRET_BASE_STASH',
                    'EVENT_OVERDUE_BOOK_NOTICE'
                ]).length;
                for (let i = 0; i < drawEventCards; i++) {
                    const drawn = p.drawPile.pop();
                    if (drawn) p.hand.push(drawn);
                }
                if (drawEventCards > 0) addEventLog(`イベントの余韻でカードを${drawEventCards}枚追加で引いた。`);

                const eventBlock = consumeAnyEventEffect([
                    'EVENT_STRAY_DOG_COMPANION',
                    'EVENT_EVACUATION_ROUTE',
                    'EVENT_POLISHED_FLOOR',
                    'EVENT_ENDLESS_ASSEMBLY',
                    'EVENT_GYM_MAT_CUSHION',
                    'EVENT_CLAY_REPLICA',
                    'EVENT_SCHOOL_BADGE_GLOW',
                    'EVENT_SHOE_LOCKER_STEP',
                    'EVENT_VAULT_JUMP',
                    'EVENT_THIRTEENTH_STEP',
                    'EVENT_WAXED_HALLWAY',
                    'EVENT_ESCAPED_RABBIT'
                ]).length * 5;
                if (eventBlock > 0) {
                    p.block += eventBlock;
                    addEventLog(`イベントの備えでブロック+${eventBlock}。`);
                }

                const eventHeal = consumeAnyEventEffect([
                    'EVENT_LEGENDARY_LUNCH',
                    'EVENT_POOL_BREATHING',
                    'EVENT_LEGEND_TREE_PROMISE',
                    'EVENT_HOME_EC_SNACK',
                    'EVENT_LEFTOVER_DUTY',
                    'EVENT_FAUCET_WATER',
                    'EVENT_WATER_TANK_RESERVE',
                    'EVENT_INFIRMARY_NAP',
                    'EVENT_EXTRA_MILK',
                    'EVENT_NEW_MENU_IDEA'
                ]).length * 3;
                if (eventHeal > 0) {
                    p.currentHp = Math.min(p.maxHp, p.currentHp + eventHeal);
                    addEventLog(`イベントの余韻でHPが${eventHeal}回復した。`);
                }

                const eventEnergy = consumeAnyEventEffect([
                    'EVENT_TRAVEL_FUND',
                    'EVENT_SECRET_BASE_PASSWORD',
                    'EVENT_TEACHERS_ROOM_NOTICE',
                    'EVENT_TEACHER_FORGOT_ITEM'
                ]).length;
                if (eventEnergy > 0) {
                    p.currentEnergy += eventEnergy;
                    addEventLog(`イベントの集中でエナジー+${eventEnergy}。`);
                }

                const eventStrength = consumeAnyEventEffect([
                    'EVENT_ANATOMY_MODEL',
                    'EVENT_SPORTS_DAY_DRILL',
                    'EVENT_EYE_TEST_FOCUS',
                    'EVENT_KITCHEN_KNIFE',
                    'EVENT_AFTER_SCHOOL_DUEL',
                    'EVENT_HELL_TRAINING'
                ]).length;
                if (eventStrength > 0) {
                    p.strength += eventStrength;
                    addEventLog(`イベントの気迫でムキムキ+${eventStrength}。`);
                }

                const upgradeEventCount = consumeAnyEventEffect([
                    'EVENT_TEST_RESULT_REVIEW',
                    'EVENT_KINJIRO_LOAD',
                    'EVENT_SECRET_NOTEBOOK',
                    'EVENT_TRASH_TREASURE',
                    'EVENT_PERFECT_SCORE_NOTICE',
                    'EVENT_SCHOOL_BROADCAST_HYPE',
                    'EVENT_FERTILE_SOIL'
                ]).length;
                if (upgradeEventCount > 0) {
                    const upgradeable = p.hand.filter(c => !c.upgraded);
                    const targets = shuffle(upgradeable).slice(0, upgradeEventCount);
                    const targetIds = new Set(targets.map(c => c.id));
                    p.hand = p.hand.map(c => targetIds.has(c.id) ? getUpgradedCard(c) : c);
                    if (targets.length > 0) addEventLog(`イベントのひらめきで手札${targets.length}枚が一時強化された。`);
                }

                const weakAllCount = consumeAnyEventEffect([
                    'EVENT_RECORDER_TUNE',
                    'EVENT_BROADCAST_JACK',
                    'EVENT_EERIE_MUSIC_ROOM',
                    'EVENT_PORTRAIT_GAZE',
                    'EVENT_PRINCIPAL_STATUE',
                    'EVENT_STRANGE_BROADCAST',
                    'EVENT_DARK_BULLETIN'
                ]).length;
                if (weakAllCount > 0) {
                    enemies.forEach(e => {
                        e.weak += weakAllCount;
                        e.floatingText = { id: `event-weak-${Date.now()}-${e.id}`, text: 'へろへろ', color: 'text-gray-400' };
                    });
                    addEventLog(`イベントの余韻で敵全体にへろへろ+${weakAllCount}。`);
                }

                const vulnAllCount = consumeAnyEventEffect(['EVENT_HUT_BOSS_AURA']).length;
                if (vulnAllCount > 0) {
                    enemies.forEach(e => {
                        e.vulnerable += vulnAllCount;
                        e.floatingText = { id: `event-vuln-${Date.now()}-${e.id}`, text: 'びくびく', color: 'text-pink-400' };
                    });
                    addEventLog(`イベントの威圧感で敵全体にびくびく+${vulnAllCount}。`);
                }

                const poisonOneCount = consumeAnyEventEffect(['EVENT_POISON_SAMPLE']).length;
                if (poisonOneCount > 0 && enemies[0]) {
                    enemies[0].poison += 3 * poisonOneCount;
                    enemies[0].floatingText = { id: `event-poison-${Date.now()}`, text: 'どく', color: 'text-purple-400' };
                    addEventLog(`イベントで得た薬品知識により、敵1体にどく+${3 * poisonOneCount}。`);
                }

                const burnAllCount = consumeAnyEventEffect(['EVENT_ALCOHOL_LAMP_FLAME', 'EVENT_LAB_EXPLOSION_RESIDUE']).length;
                if (burnAllCount > 0) {
                    enemies.forEach(e => {
                        e.currentHp = Math.max(1, e.currentHp - 3 * burnAllCount);
                        e.floatingText = { id: `event-burn-${Date.now()}-${e.id}`, text: `-${3 * burnAllCount}`, color: 'text-orange-400' };
                    });
                    addEventLog(`イベントの火種が敵全体に${3 * burnAllCount}ダメージ。`);
                }

                if (consumeEventEffect('EVENT_SHADY_MEDICINE_SAMPLE')) {
                    if (Math.random() < 0.5) {
                        p.currentEnergy += 1;
                        addEventLog('怪しい薬の残り香でエナジー+1。');
                    } else {
                        p.powers['DEXTERITY'] = (p.powers['DEXTERITY'] || 0) + 1;
                        addEventLog('怪しい薬の残り香で器用さ+1。');
                    }
                }

                if (consumeEventEffect('EVENT_MIRROR_DOUBLE')) {
                    const first = p.hand[0];
                    if (first) {
                        p.hand.push({ ...first, id: `event-mirror-${Date.now()}`, cost: 0 });
                        addEventLog('鏡の余韻で手札の先頭カードをコスト0で複製した。');
                    }
                }

                if (consumeEventEffect('EVENT_CURSED_BOOKMARK')) {
                    p.block += 3;
                    p.powers['THORNS'] = (p.powers['THORNS'] || 0) + 1;
                    addEventLog('呪われたしおりの力でブロック+3、トゲ+1。');
                }

                if (consumeEventEffect('EVENT_HUT_CLEANUP')) {
                    p.powers['DEXTERITY'] = (p.powers['DEXTERITY'] || 0) + 1;
                    addEventLog('飼育小屋を掃除した清々しさで器用さ+1。');
                }

                if (consumeEventEffect('EVENT_ROOFTOP_RAILING')) {
                    const bonus = p.currentHp <= Math.floor(p.maxHp / 2) ? 8 : 3;
                    p.block += bonus;
                    addEventLog(`屋上で覚えた踏ん張りでブロック+${bonus}。`);
                }
