...
const COOP_REALTIME_END_TURN_RESEND_MS = 1200;
const COOP_REALTIME_AUTO_ADVANCE_MS = 10000;
...
const remapCoopBattleStatePeerId = (
    ...
    return {
        ...
        players: battleState.players.map(entry =>
            entry.peerId === oldPeerId ? { ...entry, peerId: nextPeerId } : entry
        ),
        turnQueue: battleState.turnQueue.map(slot =>
            slot.peerId === oldPeerId
                ? {
                    ...slot,
                    peerId: nextPeerId,
                    id: slot.id.includes(oldPeerId) ? slot.id.replace(oldPeerId, nextPeerId) : slot.id
                }
                : slot
        ),
...
const saveCoopResumeInfoFromPayload = (payload: CoopStartPayload, selfPeerId?: string | null) => {
    if (typeof window === 'undefined') return;
    const selfParticipant =
        payload.participants.find(participant => participant.peerId === selfPeerId) ||
        payload.participants.find(participant => participant.slotId === payload.slotId) ||
        (payload.isHost ? payload.participants[0] : undefined);
    if (!payload.roomCode || !selfParticipant?.slotId || !selfParticipant.reconnectToken) return;
    window.localStorage.setItem(COOP_RESUME_STORAGE_KEY, JSON.stringify({
        roomCode: payload.roomCode,
        name: payload.name || selfParticipant.name || 'プレイヤー',
        slotId: selfParticipant.slotId,
        reconnectToken: selfParticipant.reconnectToken,
...
const loadCoopResumeInfoForMigration = (): { roomCode: string; name: string; slotId: string; reconnectToken: string } | null => {
...
const getActiveCoopParticipants = (participants: CoopParticipant[]) => {
    const activeParticipants = participants.filter(participant => !participant.disconnected);
    return activeParticipants.length > 0 ? activeParticipants : participants;
};

const areActiveCoopParticipantsResolved = (
    participants: CoopParticipant[],
    predicate: (participant: CoopParticipant) => boolean
) => {
    const activeParticipants = getActiveCoopParticipants(participants);
    return activeParticipants.length > 0 && activeParticipants.every(predicate);
};

const getNextAvailableCoopTurnCursor = (
    battleState: CoopBattleState,
    preferredCursor: number,
    participants: CoopParticipant[]
) => {
    if (battleState.turnQueue.length === 0) return 0;
    const disconnectedPeerIds = new Set(participants.filter(participant => participant.disconnected).map(participant => participant.peerId));
    const normalizedCursor = Math.max(0, preferredCursor) % battleState.turnQueue.length;
    for (let offset = 0; offset < battleState.turnQueue.length; offset++) {
        const index = (normalizedCursor + offset) % battleState.turnQueue.length;
        const slot = battleState.turnQueue[index];
        if (slot.type === 'ENEMY') return index;
        if (!slot.peerId || !disconnectedPeerIds.has(slot.peerId)) return index;
    }
    return normalizedCursor;
};

const getNextActiveCoopDecisionOwnerIndex = (participants: CoopParticipant[], currentIndex: number) => {
    if (participants.length === 0) return 0;
    for (let offset = 1; offset <= participants.length; offset++) {
        const index = (currentIndex + offset) % participants.length;
        if (!participants[index]?.disconnected) return index;
    }
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        const syncMobilePortrait = () => {
            ...
            setUiPreviewViewport({ width: window.innerWidth, height: window.innerHeight });
        };
        syncMobilePortrait();
    ...
    const dailyAssignmentNoticeTimerRef = useRef<number | null>(null);
    ...
    const markDailyAssignmentCompleted = useCallback((assignmentId: string | undefined) => {
        ...
        if (startedDailyAssignmentId === assignmentId) setStartedDailyAssignmentId(null);
    ...
    const getAssignmentProblemConfig = useCallback((assignment: AssignmentPayload | null | undefined) => {
        ...
        return {
            mode: assignmentHasCustomProblems ? GameMode.UPPER_TRIVIA : assignmentModePool ? getAssignmentRepresentativeMode(assignment) : undefined,
    ...
    const showDailyAssignmentNoticeForProblemSelection = useCallback(() => {
        if (shouldPrioritizeCurrentAssignment || !dailyAssignment) return false;
        if (dismissedDailyAssignmentId === dailyAssignment.id) return false;
        if (startedDailyAssignmentId === dailyAssignment.id) return false;
        if (dailyAssignmentNoticeTimerRef.current !== null) {
            window.clearTimeout(dailyAssignmentNoticeTimerRef.current);
        }
        setAssignmentLetterSource('selection');
        setShowAssignmentLetter(true);
        dailyAssignmentNoticeTimerRef.current = window.setTimeout(() => {
            setAssignmentLetterSource('selection');
            setShowAssignmentLetter(true);
            dailyAssignmentNoticeTimerRef.current = null;
        }, 0);
        return true;
    ...
    const coopHostBattleSyncTimerRef = useRef<number | null>(null);
    ...
    const coopHostMigrationInProgressRef = useRef(false);
    const coopHostMigrationTimerRef = useRef<number | null>(null);
    ...
    useEffect(() => {
        return () => {
            ...
            if (coopRealtimeAutoAdvanceTimerRef.current) {
                window.clearTimeout(coopRealtimeAutoAdvanceTimerRef.current);
                coopRealtimeAutoAdvanceTimerRef.current = null;
            }
            if (coopHostMigrationTimerRef.current) {
                window.clearTimeout(coopHostMigrationTimerRef.current);
                coopHostMigrationTimerRef.current = null;
            }
        };
    }, []);
    ...
    const coopDecisionOwner = useMemo(() => {
        ...
        const indexedOwner = coopSession.participants[coopSession.decisionOwnerIndex] || coopSession.participants[0] || null;
        return indexedOwner && !indexedOwner.disconnected
            ? indexedOwner
            : activeParticipants[0] || indexedOwner;
    }, [coopSession]);
    ...
    const coopEnemyHpMultiplier = useMemo(() => {
        ...
        const participantCount = coopSession ? getActiveCoopParticipants(coopSession.participants).length : 1;
    ...
    const coopAllStarterRelicsResolved = gameState.challengeMode === 'COOP' && !!coopSession && coopSession.participants.length > 0
        ? areActiveCoopParticipantsResolved(coopSession.participants, participant => !!participant.relicResolved)
        : false;
    ...
    const coopAllRestResolved = useMemo(() => {
        ...
        return areActiveCoopParticipantsResolved(coopSession.participants, participant => !!participant.restResolved);
    }, [coopSession, gameState.screen]);
    useEffect(() => {
    ...
    const getRequiredRealtimeEndPeerIds = useCallback((battleState: CoopBattleState) => {
        ...
        if (!coopSession?.isHost) return alivePeerIds;
        const connectedPeerIds = new Set([
            ...(coopSelfPeerId ? [coopSelfPeerId] : []),
            ...p2pService.getConnectedPeerIds()
        ]);
        const connectedAlivePeerIds = alivePeerIds.filter(peerId => connectedPeerIds.has(peerId));
        return connectedAlivePeerIds.length > 0 ? connectedAlivePeerIds : alivePeerIds;
    }, [coopSelfPeerId, coopSession?.isHost]);
    ...
    const getAliveCoopCompanions = useCallback(() => {
        if (!coopSession) return [];
        return coopSession.participants.filter(participant => !participant.disconnected && participant.peerId !== coopSelfPeerId && (participant.currentHp ?? participant.maxHp ?? 0) > 0);
    ...
    useEffect(() => {
        ...
        if (coopSelfPeerId && (battleState.roundEndedPeerIds || []).includes(coopSelfPeerId)) {
            coopPendingEndTurnKeyRef.current = null;
            if (coopEndTurnResendTimerRef.current) {
                window.clearInterval(coopEndTurnResendTimerRef.current);
                coopEndTurnResendTimerRef.current = null;
    ...
    const scheduleHostCoopBattleSync = useCallback((delayMs = 50) => {
        if (!coopSession?.isHost) return;
        if (coopHostBattleSyncTimerRef.current !== null) {
            window.clearTimeout(coopHostBattleSyncTimerRef.current);
        }
        coopHostBattleSyncTimerRef.current = window.setTimeout(() => {
            coopHostBattleSyncTimerRef.current = null;
            const latestState = stateRef.current;
            if (
                latestState.challengeMode !== 'COOP' ||
                latestState.screen !== GameScreen.BATTLE ||
                !latestState.coopBattleState
            ) {
                return;
            }
            broadcastCoopBattleState(latestState.coopBattleState);
        }, delayMs);
    }, [broadcastCoopBattleState, coopSession]);
    ...
    useEffect(() => {
        ...
        const activeParticipants = getActiveCoopParticipants(coopSession.participants);
        const playerSlots: CoopBattleTurnSlot[] = activeParticipants.map(participant => ({
    ...
    const sendCoopStateSync = useCallback((targetPeerId?: string) => {
        if (!isCoopHost) return;
        const payload = {
            type: 'COOP_STATE_SYNC',
            state: buildCoopSharedState(stateRef.current),
            aux: {
                shopCards,
                shopRelics,
                shopPotions,
        ...
        if (targetPeerId) {
            p2pService.sendTo(targetPeerId, payload);
        } else {
            p2pService.send(payload);
        }
    ...
    const retryWithDelay = useCallback(async <T,>(operation: () => Promise<T>, attempts = 6, delayMs = 900): Promise<T> => {
    ...
    const handleCoopHostDisconnected = useCallback((closedPeerId?: string) => {
        if (!coopSession || coopSession.isHost || !coopSession.roomCode || gameState.challengeMode !== 'COOP') return;
        const expectedHostPeerId = `lr-battle-${coopSession.roomCode}`;
        const hostParticipant = coopSession.participants[0];
        if (closedPeerId && closedPeerId !== expectedHostPeerId && closedPeerId !== hostParticipant?.peerId) return;
        if (coopHostMigrationInProgressRef.current) return;

        const selfOldPeerId = coopSelfPeerId;
        const selfParticipant = coopSession.participants.find(participant => participant.peerId === selfOldPeerId);
        const activeCandidates = coopSession.participants.filter(participant =>
            participant.peerId !== closedPeerId &&
            participant.peerId !== expectedHostPeerId &&
            !participant.disconnected
        );
        const nextHostCandidate = activeCandidates[0];
        if (!selfParticipant || !nextHostCandidate) return;

        coopHostMigrationInProgressRef.current = true;
        const roomCode = coopSession.roomCode;
        const selfResumeInfo = loadCoopResumeInfoForMigration();
        const selfSlotId = selfParticipant.slotId || selfResumeInfo?.slotId;
        const selfReconnectToken = selfParticipant.reconnectToken || selfResumeInfo?.reconnectToken;
        const selfName = selfParticipant.name || selfResumeInfo?.name || coopSession.name || 'プレイヤー';
        const isPromotedHost = nextHostCandidate.peerId === selfOldPeerId;
        ...
        setCoopSession(prev => {
            ...
            return {
                ...
                participants: prev.participants.map(participant =>
                    participant.peerId === closedPeerId || participant.peerId === expectedHostPeerId
        ...
        const startMigration = async () => {
            if (isPromotedHost) {
                const promotedPeerId = expectedHostPeerId;
                const disconnectedHostPeerId = `coop-disconnected-host-${Date.now()}`;
                const nextParticipants = coopSession.participants.map(participant => {
                    if (participant.peerId === closedPeerId || participant.peerId === expectedHostPeerId) {
                        return { ...participant, peerId: disconnectedHostPeerId, disconnected: true };
                    }
                    if (participant.peerId === selfOldPeerId) {
                        return { ...participant, peerId: promotedPeerId, disconnected: false };
                    }
                    return participant;
                });
                const nextDecisionOwnerIndex = nextParticipants.findIndex(participant => participant.peerId === promotedPeerId);

                setCoopPlayerSnapshots(prev => {
                    const next = { ...prev };
                    if (next[expectedHostPeerId]) {
                        next[disconnectedHostPeerId] = next[expectedHostPeerId];
                        delete next[expectedHostPeerId];
                    }
                    if (next[selfOldPeerId]) {
                        next[promotedPeerId] = next[selfOldPeerId];
                        delete next[selfOldPeerId];
                    }
                    return next;
                });
                ...
                const currentBattleState = stateRef.current.coopBattleState;
                if (currentBattleState) {
                    const remappedHost = remapCoopBattleStatePeerId(currentBattleState, expectedHostPeerId, disconnectedHostPeerId) as CoopBattleState | null;
                    const remappedSelf = remapCoopBattleStatePeerId(remappedHost, selfOldPeerId, promotedPeerId) as CoopBattleState | null;
                    setCoopBattleState(remappedSelf);
                }
                setCoopSession(prev => prev ? {
                    ...prev,
                    isHost: true,
                    participants: nextParticipants,
                    decisionOwnerIndex: nextDecisionOwnerIndex >= 0 ? nextDecisionOwnerIndex : prev.decisionOwnerIndex
                } : prev);
                await retryWithDelay(() => p2pService.initHost(roomCode), 8, 850);
                coopHostMigrationInProgressRef.current = false;
                window.setTimeout(() => {
                    sendCoopStateSync();
                    if (stateRef.current.screen === GameScreen.BATTLE && stateRef.current.coopBattleState) {
                        broadcastCoopBattleState(stateRef.current.coopBattleState);
                    }
                }, 200);
                return;
            }

            if (!selfSlotId || !selfReconnectToken) {
                coopHostMigrationInProgressRef.current = false;
                return;
            }
            await retryWithDelay(async () => {
                await p2pService.connect(roomCode);
                p2pService.send({
                    type: 'COOP_REJOIN',
                    name: selfName,
                    imageData: gameState.player.imageData,
                    roomCode,
                    slotId: selfSlotId,
                    reconnectToken: selfReconnectToken
                });
            }, 8, 850);
        };

        coopHostMigrationTimerRef.current = window.setTimeout(() => {
            coopHostMigrationTimerRef.current = null;
            startMigration().catch(error => {
                console.warn('Coop host migration failed', error);
                coopHostMigrationInProgressRef.current = false;
            });
    ...
    }, [gameState.player.id, gameState.player.relicCounters, gameState.screen, orreryModal]);
    ...
    const handleNodeComplete = useCallback(() => {
        ...
        if (shouldAdvanceCoopDecisionOwner) {
            setCoopSession(prev => {
                ...
                const next = {
                    ...prev,
                    decisionOwnerIndex: getNextActiveCoopDecisionOwnerIndex(prev.participants, prev.decisionOwnerIndex)
                };
                if (prev.isHost) {
        ...
        if (saved) {
            ...
            if (assignmentForContinue && isAssignmentDeadlineActive(assignmentForContinue)) {
                ...
                if (assignmentForContinue.gameMode === 'CHALLENGE_ONLY') {
                    ...
                    const assignmentConfig = getAssignmentProblemConfig(assignmentForContinue);
                    if (assignmentConfig.mode) {
                        saved.mode = assignmentConfig.mode;
                    }
    ...
    const handleStartUiPreview = useCallback((screen: GameScreen, miniGameOutcome?: MiniGameDebugPreview) => {
        ...
        setGameState(prev => {
            const previewMap = (screen === GameScreen.MAP || screen === GameScreen.BATTLE) && prev.map.length === 0
                ? generateDungeonMap(prev.difficultyLevel || 1)
                : prev.map;
            ...
            if (screen === GameScreen.FLOOR_RESULT || screen === GameScreen.ENDING || screen === GameScreen.GAME_OVER) {
                ...
                return {
                    ...
                    player: {
                        ...
                        currentHp: isGameOverPreview ? 0 : Math.max(1, prev.player.currentHp),
                    },
                    actStats: {
                        enemiesDefeated: isGameOverPreview ? 5 : 12,
                        goldGained: isGameOverPreview ? 84 : 320,
    ...
    const handleStartProblemUiPreview = useCallback((mode: GameMode, modePool?: string[]) => {
        ...
        setGameState(prev => ({
            ...
            screen,
            mode,
            modePool: modePool && modePool.length > 0 ? modePool : undefined,
            answerMode: 'CHOICE',
            challengeMode: undefined,
        }));
    ...
    }, [closeUiPreview, isUiPreviewMode]);
    ...
    const advanceCoopAfterCharacterReady = useCallback((participantsOverride?: CoopParticipant[]) => {
        ...
        if (!areActiveCoopParticipantsResolved(participants, participant => !!participant.selectedCharacterId)) return;
    ...
    const startPlayerTurn = (coopActorPeerId?: string) => {
        ...
        const isCoopHostRemoteAction = !!coopActorPeerId && currentState.challengeMode === 'COOP' && !!coopSession?.isHost;
        const latestCoopBattleState = currentState.coopBattleState || gameState.coopBattleState;
        ...
        if (gameState.challengeMode === 'COOP' && coopBattleState) {
            ...
            try {
                ...
                if (coopSession?.isHost) {
                    ...
                    nextCursor = getNextAvailableCoopTurnCursor(latestBattleState, nextCursor, coopSession.participants);
    ...
    useEffect(() => {
        ...
        const activeSlot = coopBattleState.turnQueue[coopBattleState.turnCursor];
        if (!activeSlot || activeSlot.type === 'ENEMY') return;
        const endedPeerIds = coopBattleState.roundEndedPeerIds || [];
        if (endedPeerIds.length === 0) return;
        const requiredPeerIds = getRequiredRealtimeEndPeerIds(coopBattleState);
        if (requiredPeerIds.length === 0 || requiredPeerIds.every(peerId => endedPeerIds.includes(peerId))) {
            coopRealtimeAutoAdvanceTimerRef.current = window.setTimeout(() => {
                coopRealtimeAutoAdvanceTimerRef.current = null;
                const latest = stateRef.current.coopBattleState;
                const latestSlot = latest?.turnQueue[latest.turnCursor];
                if (!latest || latest.battleMode !== 'REALTIME' || latestSlot?.type === 'ENEMY') return;
                void executeQueuedTurnTransition();
            }, 0);
            return;
        }
        const turnKey = `${coopBattleState.battleKey}:${coopBattleState.turnCursor}:${coopBattleState.enemyTurnCursor}`;
        coopRealtimeAutoAdvanceTimerRef.current = window.setTimeout(() => {
            coopRealtimeAutoAdvanceTimerRef.current = null;
            const latest = stateRef.current.coopBattleState;
            const latestSlot = latest?.turnQueue[latest.turnCursor];
            if (!latest || latest.battleMode !== 'REALTIME' || latestSlot?.type === 'ENEMY') return;
            const latestTurnKey = `${latest.battleKey}:${latest.turnCursor}:${latest.enemyTurnCursor}`;
            if (latestTurnKey !== turnKey) return;
            if ((latest.roundEndedPeerIds || []).length === 0) return;
            void executeQueuedTurnTransition();
        }, COOP_REALTIME_AUTO_ADVANCE_MS);
    }, [coopBattleState, coopSession?.isHost, executeQueuedTurnTransition, gameState.challengeMode, gameState.screen, getRequiredRealtimeEndPeerIds]);
    ...
    const handleEndTurnClick = () => {
        ...
        if (gameState.challengeMode === 'COOP' && coopSession && !coopSession.isHost) {
            ...
            if (isRealtimeCoopTurn && gameState.coopBattleState && coopSelfPeerId) {
                ...
                coopEndTurnResendTimerRef.current = window.setInterval(() => {
                    const latestBattleState = stateRef.current.coopBattleState;
                    if (
                        stateRef.current.challengeMode !== 'COOP' ||
                        stateRef.current.screen !== GameScreen.BATTLE ||
                        !latestBattleState ||
                        latestBattleState.battleMode !== 'REALTIME'
                    ) {
                        coopPendingEndTurnKeyRef.current = null;
                    ...
                    const latestTurnKey = `${latestBattleState.battleKey}:${latestBattleState.turnCursor}:${latestBattleState.enemyTurnCursor}`;
                    if (
                        latestTurnKey !== turnKey ||
    ...
    const finishRewardPhase = () => {
        ...
        if (gameState.challengeMode === 'COOP' && coopSession && coopSession.isHost) {
            ...
            if (!areActiveCoopParticipantsResolved(coopSession.participants, participant => !!participant.rewardResolved)) {
                return;
            }
        }
        if (gameState.challengeMode === 'COOP') {
    ...
    useEffect(() => {
        ...
        if (gameState.rewards.length > 0) return;
        if (!areActiveCoopParticipantsResolved(coopSession.participants, participant => !!participant.rewardResolved)) return;
        finishRewardPhase();
    ...
    useEffect(() => {
        ...
        if (!areActiveCoopParticipantsResolved(coopSession.participants, participant => !!participant.restResolved)) return;
        handleNodeComplete();
    ...
    useEffect(() => {
        ...
        if (!areActiveCoopParticipantsResolved(coopSession.participants, participant => !!participant.shopResolved)) return;
        handleNodeComplete();
    }, [coopSession, gameState.challengeMode, gameState.screen]);

    useEffect(() => {
        if (gameState.challengeMode !== 'COOP' || gameState.screen !== GameScreen.EVENT || !coopSession?.isHost) return;
        if (!areActiveCoopParticipantsResolved(coopSession.participants, participant => !!participant.eventResolved)) return;
        handleNodeComplete();
    }, [coopSession, gameState.challengeMode, gameState.screen]);

    useEffect(() => {
        if (gameState.challengeMode !== 'COOP' || gameState.screen !== GameScreen.TREASURE || !coopSession?.isHost) return;
        if (!areActiveCoopParticipantsResolved(coopSession.participants, participant => !!participant.treasureResolved)) return;
        handleNodeComplete();
    }, [coopSession, gameState.challengeMode, gameState.screen]);
    ...
    const applyHostCoopBattleSnapshot = useCallback((fromPeerId: string, payload: {
        ...
        if (options?.advanceTurn) {
            if (isRealtimeTurn && nextBattleState) {
                ...
                const alivePeerIds = getRequiredRealtimeEndPeerIds(updatedBattleState);
                const allEnded = alivePeerIds.every(peerId => nextEnded.includes(peerId));
                if (allEnded) {
                    window.setTimeout(() => {
                        void executeQueuedTurnTransition();
    ...
    useEffect(() => {
        ...
        const previousOnData = p2pService.onData;
        const previousOnClose = p2pService.onClose;
        p2pService.onClose = (closedPeerId?: string) => {
            if (coopSession.isHost && closedPeerId) {
                setCoopSession(prev => {
                    if (!prev) return prev;
                    const nextParticipants = prev.participants.map(participant =>
                ...
                const latestBattleState = stateRef.current.coopBattleState;
                const activeSlot = latestBattleState?.turnQueue[latestBattleState.turnCursor];
                if (
                    stateRef.current.screen === GameScreen.BATTLE &&
                    latestBattleState?.battleMode === 'TURN_BASED' &&
                    activeSlot?.peerId === closedPeerId
                ) {
                    window.setTimeout(() => {
                        void executeQueuedTurnTransition();
                    }, 0);
                ...
                if (
                    stateRef.current.screen === GameScreen.BATTLE &&
                    latestBattleState?.battleMode === 'REALTIME' &&
                    activeSlot?.type !== 'ENEMY'
                ) {
                    const endedPeerIds = latestBattleState.roundEndedPeerIds || [];
                    const requiredPeerIds = getRequiredRealtimeEndPeerIds(latestBattleState);
                    if (requiredPeerIds.length > 0 && requiredPeerIds.every(peerId => endedPeerIds.includes(peerId))) {
                        window.setTimeout(() => {
                            void executeQueuedTurnTransition();
                        }, 0);
            ...
            if (!coopSession.isHost) {
                handleCoopHostDisconnected(closedPeerId);
            }
        ...
        p2pService.onData = (data, fromPeerId) => {
            ...
            if (data.type === 'COOP_REJOIN' && coopSession.isHost && fromPeerId) {
                const participant = coopSession.participants.find(entry =>
                    entry.slotId === data.slotId && entry.reconnectToken === data.reconnectToken
                );
                if (!participant) {
                    p2pService.sendTo(fromPeerId, { type: 'COOP_REJOIN_REJECTED', reason: '復帰情報が一致しません' });
                    return;
                }

                const oldPeerId = participant.peerId;
                const nextParticipants = coopSession.participants.map(entry =>
                    entry.slotId === data.slotId
                        ? {
                            ...entry,
                            peerId: fromPeerId,
                ...
                setCoopSession(prev => prev ? { ...prev, participants: nextParticipants } : prev);
                setCoopPlayerSnapshots(prev => {
                    if (!oldPeerId || oldPeerId === fromPeerId || !prev[oldPeerId]) return prev;
                    const next = { ...prev, [fromPeerId]: prev[oldPeerId] };
                    delete next[oldPeerId];
                    return next;
                });
                setCoopRewardSets(prev => {
                    if (!oldPeerId || oldPeerId === fromPeerId || !prev[oldPeerId]) return prev;
                ...
                setCoopBattleQueue(prev => prev.map(slot =>
                    slot.peerId === oldPeerId
                        ? {
                            ...slot,
                            peerId: fromPeerId,
                            id: slot.id.includes(oldPeerId) ? slot.id.replace(oldPeerId, fromPeerId) : slot.id
                        }
                        : slot
                ));

                const latestBattleState = remapCoopBattleStatePeerId(
                    stateRef.current.coopBattleState ?? gameState.coopBattleState ?? null,
                    oldPeerId,
                    fromPeerId
                ) as CoopBattleState | null;
                if (latestBattleState !== (stateRef.current.coopBattleState ?? null)) {
                    setCoopBattleState(latestBattleState);
                }

                p2pService.send({ type: 'COOP_PARTICIPANTS', participants: nextParticipants, decisionOwnerIndex: coopSession.decisionOwnerIndex });
                p2pService.sendTo(fromPeerId, {
                    type: 'COOP_REJOIN_ACCEPTED',
                    roomCode: coopSession.roomCode || data.roomCode,
                    slotId: data.slotId,
                ...
                window.setTimeout(() => {
                    ...
                    const currentBattleState = stateRef.current.coopBattleState ?? null;
                    if (stateRef.current.screen === GameScreen.BATTLE && currentBattleState) {
                        broadcastCoopBattleState(currentBattleState);
                    }
                }, 120);
                return;
            }
            ...
            if (data.type === 'COOP_PARTICIPANTS') {
                ...
                if (!coopSession.isHost && gameState.screen === GameScreen.CHARACTER_SELECTION && coopSelfPeerId) {
                    const selfParticipant = data.participants.find(participant => participant.peerId === coopSelfPeerId);
                    const allCharactersSelected = areActiveCoopParticipantsResolved(data.participants, participant => !!participant.selectedCharacterId);
                    const allRelicsResolved = areActiveCoopParticipantsResolved(data.participants, participant => !!participant.relicResolved);
            ...
            if (data.type === 'COOP_BATTLE_FINISH' && !coopSession.isHost) {
                ...
                const applyBattleFinish = () => {
                    ...
                    setGameState(prev => ({
                        ...
                        screen: CHALLENGE_SCREEN_SET.has(data.screen)
                            ? data.screen
                            : (shouldPreserveLocalCoopScreen(prev.screen, data.screen) ? prev.screen : data.screen),
            ...
            if (data.type === 'COOP_BATTLE_TURN_START' && coopSession.isHost && fromPeerId && gameState.screen === GameScreen.BATTLE) {
                const latestState = stateRef.current;
                const latestBattleState = latestState.coopBattleState;
                const activeTurn = latestBattleState?.turnQueue[latestBattleState.turnCursor];
                const isRealtimeTurn = latestBattleState?.battleMode === 'REALTIME' && activeTurn?.type !== 'ENEMY';
                if (
                    latestState.screen !== GameScreen.BATTLE ||
                    !activeTurn ||
                ...
                const hostTurnKey = `${latestBattleState?.battleKey}:${latestBattleState?.turnCursor}:${latestBattleState?.enemyTurnCursor}:${activeTurn.id}:host-start:${fromPeerId}`;
                if (coopHostStartedTurnKeysRef.current.has(hostTurnKey)) {
                    broadcastCoopBattleState(latestBattleState ?? null);
                    return;
                }
                coopHostStartedTurnKeysRef.current.add(hostTurnKey);
                const actorEntry = latestBattleState?.players.find(entry => entry.peerId === fromPeerId);
                const shouldKeepInitialPreparedHand =
                    !!actorEntry &&
        ...
        return () => {
            p2pService.onData = previousOnData;
            p2pService.onClose = previousOnClose;
        };
            ...
            <div className={`w-full h-full relative overflow-hidden bg-black ${appSettings.lowDataMode ? '' : 'crt-scanline'} ${raceEffects.upsideDownUntil > raceEffectNow ? 'scale-x-[-1]' : ''} ${(raceEffects.deskShakeUntil > raceEffectNow && !appSettings.reduceScreenShake) ? 'animate-[race-desk-shake_0.18s_linear_infinite]' : ''}`}>
                ...
                {isUiPreviewMode && (
                    <div className="fixed right-2 top-2 z-[10060] flex items-start gap-2 text-white">
                        {isUiPreviewToolbarOpen ? (
                            <div className="w-[min(92vw,360px)] rounded-xl border-2 border-sky-400 bg-slate-950/95 p-3 shadow-[0_0_28px_rgba(56,189,248,0.5)] backdrop-blur-md">
                            ...
                            <button
                                type="button"
                                onClick={() => setIsUiPreviewToolbarOpen(true)}
                                className="flex items-center gap-2 rounded-lg border-2 border-sky-400 bg-slate-950/95 px-3 py-2 text-xs font-black text-sky-200 shadow-lg"
                            >
                                <Monitor size={16} /> {uiPreviewViewport.width}×{uiPreviewViewport.height}
                            </button>
                        )}
                    </div>
                )}
                ...
                {showTimeLimitModal && (
                    <div className="app-modal-overlay app-time-limit-modal-overlay fixed inset-0 z-[10000] bg-black/90 flex items-center justify-center p-4 animate-in fade-in duration-300">
                        <div className="app-modal-panel app-time-limit-modal bg-gray-900 border-4 border-red-600 p-8 rounded-2xl max-sm w-full shadow-[0_0_50px_rgba(220,38,38,0.5)] text-center transform scale-110">
                            ...
                            <h2 className="text-3xl font-black text-white mb-4 tracking-tighter">{trans("時間切れ！", languageMode)}</h2>
                            ...
                            >
                                <Check size={24} /> {trans("わかった！", languageMode)}
                            </button>
                        </div>
                    </div>
                )}
                        ...
                        <div className="absolute top-2 right-2 z-[10010] flex items-center gap-1.5 sm:gap-2">
                            ...
                            {isElectronApp && (
                                <button
                                    ...
                                    title={trans("ゲームをとじる", languageMode)}
                                >
                                    <X size={16} />
                                </button>
                            )}
                        ...
                        <div className="start-menu-content relative z-10 text-center p-8 w-full flex flex-col items-center">
                            ...
                            {isAssignmentChallengeOnlyLocked && activeAssignment && (
                                <div className="mb-3 w-full max-w-[360px] rounded-lg border border-emerald-400 bg-emerald-950/80 px-3 py-2 text-center text-xs font-black text-emerald-100 shadow-lg">
                                    {trans("課題期限内のため「問題チャレンジのみ」利用できます", languageMode)}
                            ...
                            <div className="start-menu-button-panel flex w-full flex-col items-center">
                                <div className="mb-4 flex items-stretch justify-center gap-2">
                                    {assignmentLetter && isAssignmentLetterWithinDeadline && (
                                        ...
                                        >
                                            {trans(isTeacherAssignmentActive ? '課題レター' : '今日の課題', languageMode)}
                                        </button>
                                    )}
                                    <div className="start-menu-theme-switch inline-flex overflow-hidden border-2 border-slate-500 bg-black/55 shadow-lg">
                                        {([
                                            { id: 'elementary', label: trans('小学生編', languageMode) },
                                            { id: 'high-school', label: trans('高校編', languageMode) },
                                            { id: 'magic', label: trans('マジック編', languageMode) },
                                        ] as const).map((theme) => (
                                        <button
                                            key={theme.id}
                                            type="button"
                ...
                {raceSession && !raceSession.ended && raceSession.isHost && gameState.challengeMode === 'RACE' && gameState.screen !== GameScreen.RACE_SETUP && raceSession.roomCode && (
                    <div className="absolute left-1/2 top-3 z-40 -translate-x-1/2">
                        <div className="bg-slate-900/90 border border-cyan-500 rounded-lg px-3 py-2 text-cyan-100 shadow-lg">
                            <div className="text-center text-[10px] font-bold tracking-wide text-cyan-200">{trans("参加コード", languageMode)}</div>
                ...
                {gameState.challengeMode === 'COOP' && coopSession && COOP_DECISION_HUD_SCREEN_SET.has(gameState.screen) && coopDecisionOwner && (
                    <div className="absolute right-2 sm:right-3 top-[60px] sm:top-[72px] z-30">
                        <div className="bg-slate-900/90 border border-cyan-500 rounded-lg px-2 py-1.5 sm:px-3 sm:py-2 text-cyan-100 shadow-lg min-w-[116px] sm:min-w-[168px] max-w-[42vw] sm:max-w-none">
                            <div className="text-[9px] sm:text-[10px] font-bold tracking-wide text-cyan-200">{trans("決定役", languageMode)}</div>
                            <div className="truncate text-[11px] sm:text-sm font-black">{coopDecisionOwner.name}</div>
                            <div className="text-[9px] sm:text-[11px] text-cyan-100/80">{trans(coopCanDecide ? 'あなたの番です' : '進行待ち', languageMode)}</div>
                        </div>
                    </div>
                )}
                                                ...
                                                <div className="flex items-center justify-between gap-2 text-[9px] sm:text-[10px] text-slate-300">
                                                    ...
                                                    {gameState.screen === GameScreen.REWARD ? (
                                                        <span className={`truncate text-right ${participant.rewardResolved ? 'text-yellow-300' : 'text-slate-300'}`}>
                                                            {trans(participant.rewardResolved ? '報酬完了' : '報酬中', languageMode)}
                                                        </span>
                                                        ...
                                                        <span className={`truncate text-right ${participant.shopResolved ? 'text-yellow-300' : 'text-slate-300'}`}>
                                                            {trans(participant.shopResolved ? '買い物完了' : '買い物中', languageMode)}
                                                        </span>
                                                        ...
                                                        <span className={`truncate text-right ${participant.restResolved ? 'text-yellow-300' : 'text-slate-300'}`}>
                                                            {trans(participant.restResolved ? '休憩完了' : '休憩中', languageMode)}
                                                        </span>
                                                    ) : gameState.screen === GameScreen.EVENT ? (
                                                        <span className={`truncate text-right ${participant.eventResolved ? 'text-yellow-300' : 'text-slate-300'}`}>
                                                            {trans(participant.eventResolved ? 'イベント完了' : 'イベント中', languageMode)}
                                                        </span>
                                                        ...
                                                        <span className={`truncate text-right ${participant.treasureResolved ? 'text-yellow-300' : 'text-slate-300'}`}>
                                                            {trans(participant.treasureResolved ? '宝確認完了' : '宝確認中', languageMode)}
                                                        </span>
                                                        ...
                                                        <span className={`truncate text-right ${participant.quizResolved ? 'text-yellow-300' : 'text-slate-300'}`}>
                                                            {participant.quizResolved ? `${trans('クイズ', languageMode)} ${participant.quizCorrectCount ?? 0}` : trans('クイズ中', languageMode)}
                                                        </span>
                                                    ) : (
                                                        <span className={`truncate text-right ${isDecisionOwner ? 'text-cyan-300' : 'text-slate-400'}`}>
                                                            {trans(isDecisionOwner ? '進行中' : '待機', languageMode)}
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
                            ...
                            <div className="bg-slate-950/88 border border-fuchsia-500/70 rounded-xl shadow-2xl backdrop-blur px-3 py-2 text-white">
                                ...
                                >
                                    <div>
                                        ...
                                        <div className="text-sm font-black">{trans("妨害カード", languageMode)} {raceTrickCards.length} {trans("枚", languageMode)}</div>
                                    </div>
                                    ...
                                    <div className="mt-3 space-y-2 max-h-[40vh] overflow-y-auto custom-scrollbar pr-1">
                                        {raceTrickCards.length === 0 && (
                                            <div className="rounded border border-slate-700 bg-slate-900/80 px-3 py-2 text-xs text-slate-300">
                                                {trans("戦闘報酬でたまに手に入るレース専用カードです。", languageMode)}
                                            </div>
                                        )}
                                                ...
                                                <div key={`${card.id}-${index}`} className="rounded-lg border border-fuchsia-500/30 bg-fuchsia-950/25 p-3">
                                                    ...
                                                    <div className="mt-2 flex flex-wrap gap-2">
                                                        {card.effectId === 'WALLET_SWAP' ? (
                                                            <>
                                                                {targets.length === 0 && <div className="text-[11px] text-slate-400">{trans("対象なし", languageMode)}</div>}
                                                                {targets.map(target => (
                                                                    <button
                                                                    ...
                                                                    >
                                                                        {target.name} {trans("に使う", languageMode)}
                                                                    </button>
                                                                ))}
                                                            </>
                                                        ) : (
                        ...
                        {OFFLINE_DISTRIBUTABLE ? (
                            ...
                            <CoopSetupScreen
                                ...
                                onStart={(payload: CoopStartPayload) => {
                                    ...
                                    saveCoopResumeInfoFromPayload(payload, selfPeerId);
