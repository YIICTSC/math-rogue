...
const bossRows: Array<[EndlessArc, number, EndlessBossTier, string, string, string, string]> = [
  ...
  ['elementary', 20, 'MAJOR_BOSS', '三相演算体オルビス', '三つの演算相を回す中枢', 'SUBJECT_TRIAD', '同一戦闘内で異なるsubjectIdの学習判定に成功すると相が崩れる3フェーズ。'],
  ...
  ['elementary', 40, 'MAJOR_BOSS', '反復機リピタ', '同じ記録を再生する機械体', 'REPEAT_RECORD', '同一戦闘内で学習判定に3回連続成功すると停止する。直前のカード効果は弱く再演される。'],
  ['elementary', 45, 'BOSS', '空白面ブランクス', '空白の仮面だけが浮く存在', 'UNDEFINED', '未定義タグを付与し、同一戦闘内の学習判定に成功すると空白を復元する。'],
  ...
  ['magic', 35, 'BOSS', '深層頁獣アビスラ', '頁状パネルが脚になる異形', 'SEALED_PAGE', 'カードを封印し、同一戦闘内で対応する学習判定に成功すると回収する。'],
