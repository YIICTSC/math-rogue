...
const bossRows: Array<[EndlessArc, number, EndlessBossTier, string, string, string, string]> = [
  ...
  ['elementary', 40, 'MAJOR_BOSS', '反復機リピタ', '同じ記録を再生する機械体', 'REPEAT_RECORD', '直前のカード効果を弱く再演し、連続成功で停止する。'],
...
const rewardRows: Record<EndlessArc, Record<number, Array<[EndlessRewardSlot, string, string, EndlessRewardScope, string]>>> = {
  elementary: {
    ...
    10: [['CORE', '採点保護膜', '最初の学習判定失敗による追加ペナルティを無効化', 'RUN', 'IGNORE_FIRST_LEARNING_FAIL'], ['GROWTH', '三相演算板', '次のカード報酬を異なるカードタイプ3種から選ぶ', 'RUN', 'TRIPLE_CARD_TYPES'], ['CONTRACT', '赤線逆流核', 'HP30％以下で次に解決するダメージ付きATTACKの最終ダメージを2倍。ただしHP5', 'RUN', 'LOW_HP_DOUBLE_ATTACK']],
    15: [['SAFE', '暗号ゲートの笛', '戦闘開始時、敵の最初の行動内容を表示', 'RUN', 'REVEAL_FIRST_INTENT'], ['LEARNING', '解除手順メモ', '同一subjectIdの学習判定に3回連続成功するとエネルギー1', 'RUN', 'SUBJECT_STREAK_ENERGY'], ['RISK', 'オクト跳躍鍵', 'ゲート解除判定成功後、次に出る小危険ノードを1回無視。ただしG20', 'RUN', 'SKIP_MINOR_NODE']],
  ...
  'high-school': {
    ...
    30: [['CORE', '単独演算バンド', '学習判定連続成功、または同じカードタイプ連続使用1回ごとに、次の3戦闘の与ダメージを10％上げる（最大3回）', 'RUN', 'COMBO_DAMAGE'], ['GROWTH', 'コンボ設計図', '各戦闘開始時、コンボ数1から開始', 'RUN', 'START_COMBO'], ['CONTRACT', '越境コード', '異なる2カードタイプを連続使用するとエネルギー1。ただし次ターンはドロー不可', 'RUN', 'TYPE_CHAIN_ENERGY']],
    35: [['SAFE', '評定メモリ', '次のマップでルートごとの報酬傾向を表示', 'RUN', 'REVEAL_ROUTE_REWARDS'], ['LEARNING', '均衡評価表', '同一戦闘内の攻撃カード使用数・ブロックカード使用数・学習判定成功数の差を1以内にそろえると最大HP3', 'RUN', 'BALANCE_MAX_HP'], ['RISK', '逆評定コード', '次戦闘で最初に付く不利効果1つを同量の有利効果へ反転。ただし次戦闘開始時に弱体1', 'RUN', 'REVERSE_DEBUFF']],
  ...
  magic: {
    5: [['SAFE', 'テンポラの差分キー', '各戦闘1回、直前に解決したカードの効果を取り消し、同じコストで選び直す', 'RUN', 'RETRY_CARD'], ['LEARNING', '時相ログ', '戦闘内で最初に別の属性IDへ切り替えた時、1枚ドロー', 'RUN', 'DRAW_ATTRIBUTE_SWITCH'], ['RISK', '加速核', '1戦闘1回、プレイヤーターンを1回追加。ただし腐蝕1', 'RUN', 'EXTRA_TURN_CORRUPTION']],
    ...
    35: [['SAFE', 'アビスラの栞鍵', '各戦闘1回、捨て札または封印カードを手札へ戻す', 'RUN', 'RETURN_DISCARD'], ['LEARNING', '未読記録', '各戦闘で封印カードの解除判定に最初に成功した学習判定の直後、ドロー1＋ブロック4', 'RUN', 'SEALED_LEARNING_REWARD'], ['RISK', '頁獣再封印', '呪いカードを1枚除去。ただし最もレアリティの高いカードを1枚封印', 'RUN', 'REMOVE_CURSE_SEAL_RARE']],
