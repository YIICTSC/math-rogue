...
import { CharacterAppearanceMode, Enemy, Player, Card as ICard, CardType, SelectionState, Potion, FloatingText, EnemyIntentType, LanguageMode, ParryState, VisualEffectInstance, CoopSupportCard, RaceTrickCard, AttackEffectKey, ActiveFamiliar } from '../types';
...
import { getVacationAnimationFrameTranslation } from '../data/vacationAnimationAnchors.generated';
...
const getVacationAnimationAnchorStyle = (source: string | null, enabled: boolean, mirrored: boolean): React.CSSProperties => {
    if (!enabled || !source) return {};
    const style: Record<string, string> = {};
    for (let frame = 0; frame < 4; frame += 1) {
        const translation = getVacationAnimationFrameTranslation(source, frame);
...
const BattleScene: React.FC<BattleSceneProps> = ({
    ...
    const shouldShowMagicTransformationTutorial = visualTheme === 'magic' && !storageService.getSeenMagicTransformationTutorial();
    ...
    const shouldAnchorVacationAnimation = player.appearanceMode === 'VACATION'
        && (shouldRenderIdleSpriteSheet || shouldRenderHeroAnimationSheet);
    const vacationAnimationAnchorStyle = getVacationAnimationAnchorStyle(
        shouldRenderHeroAnimationSheet ? heroAnimationSheetSource : idleSpriteSheetSource,
        shouldAnchorVacationAnimation,
            ...
            {showMagicTransformationTutorial && tutorialStep === null && (
                <div data-gamepad-modal data-gamepad-initial-scope="magic-transform-tutorial" className="app-modal-overlay magic-transform-tutorial-overlay fixed inset-0 z-[205] flex items-center justify-center bg-slate-950/82 p-4 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="app-modal-panel magic-transform-tutorial-panel w-full max-w-3xl overflow-hidden rounded-2xl border-2 border-fuchsia-300/80 bg-slate-950 shadow-[0_0_42px_rgba(217,70,239,0.55)]">
                        ...
                        <div className="magic-transform-tutorial-body space-y-3 p-4 text-sm leading-relaxed text-slate-100 md:p-5">
                            {languageMode === 'ENGLISH' ? (
                                <>
                                ...
                                >
                                    ...
                                    <div className={`battle-enemy-sprite ${humanoidEnemy ? 'battle-humanoid-enemy-sprite' : ''} ${useFinalBossLayout ? 'battle-final-boss-sprite' : ''} ${(enemy.enemyType === 'DODOMEDESU' || enemy.enemyType === 'GENZO') && isDodomedesuBattle ? 'invisible absolute' : ''} relative mb-1 transition-all duration-700 ${enemySpriteSizeClass}`}>
                                        {isFinisherActive ? (
                                            ...
                                            <EnemyIllustration name={enemy.name} altText={enemyName} seed={enemy.id} aliases={enemySvgAliases} visualTheme={visualTheme} enemyType={enemy.enemyType} phase={enemy.phase} action={highSchoolEnemyAction} appearanceMode={player.appearanceMode} className="w-full h-full drop-shadow-lg relative z-10" />
                                        )}
                            ...
                            <div ref={playerSpriteRef} className={`battle-player-sprite order-1 ${visualTheme !== 'elementary' ? 'w-40 h-40 md:w-48 md:h-48' : 'w-20 h-20 md:w-24 md:h-24'} relative transition-all duration-150 ease-out ${isTrueBossPhase2SpecialLayout ? 'mr-0 md:mr-2 mb-1 md:mb-0' : 'mr-2'} ${heroActionClass} ${selectedSupportCard ? 'ring-2 ring-emerald-300 rounded-lg cursor-pointer' : ''}`} onClick={() => {
                                ...
                                {shouldRenderHeroAnimationSheet ? (
                                    <div
                                        ...
                                        className={`${heroAnimationSheetClassName} ${shouldAnchorVacationAnimation ? 'battle-hero-vacation-anchored-sprite-sheet' : ''} relative z-10 w-full h-full ${visualTheme === 'high-school' ? '-scale-x-100' : ''}`}
                                        style={{
                                    ...
                                    <div
                                        ...
                                        role="img"
                                        aria-label={trans('主人公', languageMode)}
                                        className={`battle-hero-idle-sprite-sheet ${shouldAnchorVacationAnimation ? 'battle-hero-vacation-anchored-sprite-sheet' : ''} relative z-10 w-full h-full ${visualTheme === 'high-school' ? '-scale-x-100' : ''}`}
                                        style={{
