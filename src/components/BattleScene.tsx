...
const FloatingTextOverlay: React.FC<{ data: FloatingText | null }> = ({ data }) => {
    if (!data) return null;
    return (
        <div key={data.id} className={`absolute -top-8 left-1/2 -translate-x-1/2 font-bold text-2xl animate-bounce ${data.color} z-50 drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]`}>
             {data.text}
        </div>
    );
};
  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      <div className="flex-grow bg-gray-900 relative overflow-hidden flex justify-between items-center px-2 md:px-4 py-4 bg-[url('https://picsum.photos/800/400?grayscale&blur=2')] bg-cover bg-blend-multiply">
        ...
        {/* Player Sprite & Stats */}
        <div className="flex flex-col items-center z-10 w-1/3 md:w-1/4 relative">
            <FloatingTextOverlay data={player.floatingText} />
            <div className="mb-2 w-full md:w-48 bg-black/80 border-2 border-white p-2 text-white text-[10px] md:text-xs">
                ...
                {/* Relics & Potions Row */}
                <div className="flex flex-wrap gap-1 mb-2 border-b border-gray-700 pb-1">
                    {/* Relics */}
                    <div className="flex gap-1 overflow-x-auto max-w-[50%] no-scrollbar">
                        {player.relics.map(r => (
                            <div key={r.id} className="w-4 h-4 md:w-5 md:h-5 bg-gray-700 rounded-full border border-yellow-600 flex items-center justify-center relative group shrink-0" title={r.description}>
                ...
                <div className="w-full h-1.5 md:h-2 bg-gray-700 rounded-full border border-gray-500 overflow-hidden">
            ...
            <div className={`w-24 h-24 md:w-32 md:h-32 relative transition-all duration-150 ease-out ${getActionClass()}`}>
                 <img 
                    src={player.imageData || HERO_IMAGE_DATA} 
                    alt="Hero" 
        ...
        {/* Enemies */}
        <div className="flex justify-center items-end gap-2 w-2/3 md:w-3/4 pr-2 h-full pb-8">
            {enemies.map((enemy) => {
                ...
                return (
                    <div 
                        ...
                        className={`flex flex-col items-center z-10 transition-transform duration-200 cursor-pointer relative ${isSelected && !actionClass ? 'scale-110' : ''} ${!isSelected && !actionClass ? 'hover:scale-105' : ''} ${actionClass}`}
                    >
                        ...
                        <div className={`mb-2 w-24 md:w-28 bg-black/80 border-2 p-1 text-white text-[9px] md:text-[10px] transition-colors ${isSelected ? 'border-yellow-400 shadow-[0_0_10px_rgba(255,255,255,0.5)]' : 'border-red-500'}`}>
                            <div className="flex items-center justify-between mb-1">
      ...
      <div className={`h-48 md:h-64 bg-gray-900 border-t-4 border-white p-2 md:p-4 flex flex-col relative z-20 ${selectionState.active ? 'shadow-[0_-5px_20px_rgba(255,255,0,0.3)]' : ''}`}>
        ...
        <div className="absolute -top-4 md:-top-6 left-2 md:left-4 flex space-x-1 md:space-x-2">
            ...
            <button 
                ...
                className="bg-black border-2 border-blue-500 text-blue-400 px-2 py-0.5 md:px-3 md:py-1 rounded-full text-[10px] md:text-xs flex items-center hover:bg-blue-900/50 hover:text-white transition-colors cursor-pointer"
            >
                <Layers size={12} className="mr-1"/>
                デッキ: {player.deck.length}
            </button>
            <div className="bg-black border-2 border-gray-500 text-gray-400 px-2 py-0.5 md:px-3 md:py-1 rounded-full text-[10px] md:text-xs flex items-center">
                捨札: {player.discardPile.length}
            </div>
        </div>

        {/* End Turn Button */}
        <div className="absolute -top-4 md:-top-6 right-2 md:right-4">
            <button 
                onClick={!actingEnemyId && !selectionState.active ? onEndTurn : undefined}
                disabled={!!actingEnemyId || selectionState.active}
                className={`bg-red-600 border-2 border-white px-3 py-1 md:px-6 md:py-2 text-xs md:text-sm shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] md:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all
                  ${!actingEnemyId && !selectionState.active ? 'hover:bg-red-500 active:shadow-none active:translate-x-[2px] active:translate-y-[2px] cursor-pointer' : 'opacity-50 cursor-not-allowed'}
                `}
            >
        ...
        {/* Cards */}
        <div className="flex-grow flex items-center justify-center gap-1 md:gap-2 px-1 md:px-8 overflow-x-auto pb-4 pt-2">
            {player.hand.map((card) => (
                <div key={card.id} className={`shrink-0 ${selectionState.active ? 'cursor-pointer hover:scale-110 transition-transform' : ''}`}>
                    <Card 
                        card={card} 
                        onClick={() => {
