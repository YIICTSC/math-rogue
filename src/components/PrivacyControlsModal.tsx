  ...
  return <div data-gamepad-modal data-gamepad-initial-scope="privacy-controls" className="fixed inset-0 z-[10090] flex items-center justify-center bg-black/90 p-3" role="dialog" aria-modal="true" aria-labelledby="privacy-controls-title">
    <section className="max-h-[calc(100dvh-1.5rem)] w-full max-w-2xl overflow-y-auto rounded-2xl border-4 border-cyan-300 bg-slate-950 p-5 text-white sm:p-7">
      <header className="flex items-start justify-between gap-3">
        <div><ShieldCheck className="mb-2 text-cyan-300" size={34} /><h2 id="privacy-controls-title" className="text-2xl font-black">{t('プライバシーとデータ')}</h2></div>
      ...
      <div className="mt-5 grid gap-2 sm:grid-cols-2">
        ...
        {onlineProfile && <button disabled={busy} onClick={() => void deleteRanking()} className="flex items-center justify-center gap-2 rounded-xl border border-rose-500 bg-rose-950 px-4 py-3 font-black text-rose-100"><Trash2 size={17} />{t('ランキングデータを削除')}</button>}
        <button disabled={busy} onClick={clearLocal} className="flex items-center justify-center gap-2 rounded-xl border border-rose-700 bg-slate-900 px-4 py-3 font-black text-rose-200"><Trash2 size={17} />{t('この端末の全データを削除')}</button>
      </div>
      ...
      <div className="mt-5 flex flex-wrap gap-3 text-xs font-bold text-cyan-300">
        <a href="./privacy.html" target="_blank" rel="noreferrer" className="flex items-center gap-1">{t('プライバシーポリシー')} <ExternalLink size={13} /></a>
        <a href="./delete-data.html" target="_blank" rel="noreferrer" className="flex items-center gap-1">{t('データ削除方法')} <ExternalLink size={13} /></a>
      </div>
    </section>
  </div>;
}
