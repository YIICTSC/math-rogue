...
const SchoolyardSurvivorScreen: React.FC<SchoolyardSurvivorScreenProps> = ({ onBack, problemMode = GameMode.MIXED, problemModePool, answerMode = 'CHOICE', assignment, onAnswerResult, languageMode = 'JAPANESE', debugPreview }) => {
    ...
    return (
        <div className="mini-game-survivor-screen flex flex-col h-full w-full bg-[#020617] text-white relative items-center justify-center font-mono overflow-hidden touch-none select-none" ref={containerRef} onTouchStart={(e) => { if (gameState.current !== 'PLAYING') return; const t = e.touches[0]; setJoystickUI({ active: true, startX: t.clientX, startY: t.clientY, curX: t.clientX, curY: t.clientY }); joystickRef.current = { x: 0, y: 0 }; }} onTouchMove={(e) => { if (!joystickUI?.active) return; const t = e.touches[0]; const dx = t.clientX - joystickUI.startX; const dy = t.clientY - joystickUI.startY; const dist = Math.hypot(dx, dy); const max = 50; setJoystickUI(prev => prev ? ({ ...prev, curX: prev.startX + (dist>max ? dx/dist*max : dx), curY: prev.startY + (dist>max ? dy/dist*max : dy) }) : null); joystickRef.current = { x: dx/max, y: dy/max }; }} onTouchEnd={() => { setJoystickUI(null); joystickRef.current = { x: 0, y: 0 }; }}>
            ...
            {gameState.current === 'MATH_CHALLENGE' && (<div className="absolute inset-0 z-30 pointer-events-auto"><MiniGameProblemChallenge mode={problemMode} modePool={problemModePool} answerMode={answerMode} assignment={assignment} onAnswerResult={onAnswerResult} onComplete={handleMathComplete} rewardHint={tr('全問正解でHP15回復')} languageMode={languageMode} /></div>)}
            {gameState.current === 'LEVEL_UP' && (
                    ...
                    <div className="grid grid-cols-1 gap-4 w-full max-md overflow-y-auto max-h-[70vh] p-2 custom-scrollbar">
                        {upgradeOptions.map((opt, idx) => {
                            ...
                            const weaponIconStyle = opt.type === 'WEAPON'
                                ...
                                : undefined;
                            return (
                                <button key={idx} onClick={() => selectUpgrade(opt)} className={`bg-slate-900 border-4 ${isEvo ? 'border-yellow-400 bg-yellow-900/40 shadow-[0_0_20px_rgba(250,204,21,0.3)]' : 'border-slate-700'} hover:border-blue-400 hover:bg-slate-800 p-4 rounded-2xl flex items-center text-left transition-all group relative overflow-hidden shadow-2xl`}><div className={`w-16 h-16 bg-black/60 mr-4 ${isEvo ? 'animate-bounce' : ''} shrink-0 p-2 border-2 border-slate-600 rounded-xl shadow-inner`}>{weaponIconStyle ? <div className="w-full h-full" style={weaponIconStyle} /> : <PixelSprite seed={opt.id} name={`${itemDef.sprite.template}|${itemDef.sprite.color}`} className="w-full h-full"/>}</div><div className="flex-1"><div className="flex justify-between items-center mb-1"><div className={`text-xl font-black leading-tight ${isEvo ? 'text-yellow-300' : 'text-white'}`}>{tr(isEvo ? itemDef.evolvedName : itemDef.name)}</div><div className="text-[10px] text-blue-300 font-black bg-blue-900/60 px-3 py-1 rounded-full border border-blue-400/50 uppercase">{opt.isNew ? tr('New Weapon') : (opt.type === 'HEAL' ? tr('Bonus') : `Lv ${opt.level || 'Master'}`)}</div></div><div className="text-xs text-gray-300 leading-snug font-bold">{tr(isEvo ? itemDef.evolvedDesc : itemDef.desc)}</div>{opt.type === 'WEAPON' && !isEvo && <div className="text-[10px] text-indigo-400 mt-1 flex items-center gap-1 font-black"><Sparkles size={10}/> {tr('SYNERGY')}: {tr(PASSIVES[itemDef.synergy as PassiveType].name)}</div>}</div>{isEvo && <div className="absolute top-0 right-0 bg-gradient-to-b from-yellow-400 to-orange-600 text-black text-[10px] font-black px-4 py-1 rounded-bl-xl shadow-lg">{tr('協助進化')}</div>}</button>
                            );
                        })}
                    </div>
                </div>
            ...
            {uiState.gameOver && (<div className="absolute inset-0 bg-red-950/95 flex flex-col items-center justify-center z-20 pointer-events-auto animate-in fade-in duration-500"><Skull size={80} className="text-red-500 mb-4 animate-bounce"/><h2 className="text-7xl font-black text-white mb-4 tracking-tighter italic">{tr('GAME OVER')}</h2><div className="text-2xl text-yellow-400 mb-8 font-black bg-black/60 px-8 py-2 rounded-full border border-yellow-500">{tr('SURVIVED')}: {Math.floor(uiState.time/60)}:{(uiState.time%60).toString().padStart(2,'0')}</div><div className="flex flex-col gap-4 w-72"><button onClick={handleRestart} className="bg-white text-black px-8 py-4 rounded-2xl font-black text-xl hover:bg-gray-200 flex items-center justify-center shadow-[0_4px_0_#ccc] active:translate-y-1 active:shadow-none transition-all"><RotateCcw className="mr-2"/> {tr('PLAY AGAIN')}</button><button onClick={handleExit} className="bg-black text-white px-8 py-4 rounded-2xl font-black text-xl border-4 border-white/20 hover:bg-gray-900 flex items-center justify-center shadow-2xl transition-all"><ArrowLeft className="mr-2"/> {tr('EXIT TO MENU')}</button></div></div>)}
            {gameState.current === 'PLAYING' && (<button onClick={handleExit} className="survivor-pause-button absolute top-4 right-4 bg-gray-800/60 hover:bg-red-500/80 text-white p-3 rounded-2xl border-2 border-white/20 z-50 pointer-events-auto shadow-2xl backdrop-blur-lg transition-all"><Pause size={24} /></button>)}
        </div>
    );
};
