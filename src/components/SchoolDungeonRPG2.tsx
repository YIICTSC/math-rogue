...
const SchoolDungeonRPG2: React.FC<SchoolDungeonRPG2Props> = ({ onBack }) => {
  ...
  return (
    <div className="w-full h-full bg-[#101010] flex flex-col items-center justify-center font-mono select-none overflow-hidden touch-none relative p-4 gap-4">
        ...
        {/* Game Area (Top) */}
        <div className="w-full max-w-lg aspect-[4/3] relative shrink-0">
        ...
        {/* Controls Container - Split Left/Right */}
        <div className="w-full max-w-lg flex-1 min-h-[300px] relative bg-[#1a1a1a] rounded-xl border-2 border-[#333] shadow-2xl flex overflow-hidden">
            
            {/* Quit Button (Top Right Absolute) */}
            ...
            {/* Left Zone: D-Pad */}
            <div className="w-1/2 h-full relative flex items-center justify-center border-r border-[#333]">
                {/* D-Pad */}
            ...
            {/* Right Zone: Actions & Cards */}
            <div className="w-1/2 h-full relative flex flex-col justify-end items-end p-2 pb-4 gap-4">
                
                {/* Buttons Row (Menu, Shoot, Action) */}
                <div className="flex gap-4 items-end mb-2 mr-2">
                     {/* Shoot */}
                     <div className="flex flex-col items-center group">
                        <button className="w-12 h-12 bg-[#333] rounded-full shadow-[0_2px_0_#111] active:shadow-none active:translate-y-1 transition-all flex items-center justify-center text-white border border-[#555] touch-none select-none" onClick={fireRangedWeapon}><Crosshair size={20}/></button>
                        <span className="text-[#666] text-[9px] font-bold mt-1">R</span>
                    </div>
                     {/* Menu (B) */}
                     <div className="flex flex-col items-center group">
                ...
                {/* Cards Row */}
                <div className="flex gap-2 justify-end items-end w-full h-32 pr-2">
                     {dungeonHand.length > 0 ? (
                        dungeonHand.map((card, i) => (
                            <button 
                                key={card.id} 
                                className="w-16 h-24 bg-[#2a2a2a] border-2 border-[#555] rounded-lg flex flex-col items-center justify-start text-white relative shadow-lg hover:bg-[#333] hover:border-white hover:-translate-y-2 active:scale-95 transition-all p-1 overflow-hidden"
                                onClick={() => handleCardUse(i)}
                            >
                                <div className={`w-full text-[6px] font-bold px-1 rounded-t mb-1 text-center ${card.type === 'ATTACK' ? 'bg-red-900 text-red-200' : card.type === 'DEFENSE' ? 'bg-blue-900 text-blue-200' : card.type === 'BUFF' ? 'bg-green-900 text-green-200' : 'bg-purple-900 text-purple-200'}`}>
                                    {card.type}
                                </div>
                                <div className="mb-1 p-1 bg-black/50 rounded-full">{card.icon}</div>
                                <div className="text-[8px] font-bold text-center leading-tight w-full break-words px-0.5">{card.name}</div>
                                <div className="text-[8px] text-gray-400 mt-auto border-t border-[#444] w-full text-center pt-0.5">{card.power > 0 ? `Pow:${card.power}` : 'Effect'}</div>
                            </button>
                        ))
                    ) : (
