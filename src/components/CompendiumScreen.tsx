...
const HIGH_SCHOOL_CHARACTER_DESCRIPTION_OVERRIDES: Record<string, string> = {
    '監督官の先輩': '後輩の行動を見守るという名目で、失敗だけは見逃さない先輩。監督の目が鋭すぎて、こちらの言い訳まで採点済み。',
    '剣道部の風紀委員': '竹刀と校則を同時に振り回す風紀委員。制服の乱れより、こちらの作戦の乱れを先に見つけてくる。',
    '白衣の査問官': '白衣を着た瞬間、あらゆる事故を「想定内」と言い張る査問官。質問が終わる頃には、なぜかこちらが被告。',
    '禁書管理の書記': '禁書の貸出記録を一冊ずつつける几帳面な書記。世界を滅ぼす本より、返却期限のほうを重く見ている。',
    '銀髪の審査員': '採点基準が本人の気分で変わる審査員。満点を取っても「伸びしろ」と言われるので、勝っても褒められない。',
    '鎖の執行部員': '校則を鎖にして実体化する執行部員。自由を奪う手際だけは見事で、規則の穴を探す時間もくれない。',
    '赤章の副会長': '議事録にない決定まで赤章ひとつで通す副会長。反論は受け付けるが、採用するとは言っていない。',
    '実験区画の主任': '実験の安全確認を誰かに任せ、自分は結果だけを見に来る主任。爆発すれば成功、静かなら再実験らしい。',
    '表彰台の王子': '表彰状を盾にしてくる校内の王子。過去の栄光を語る間だけ無敵で、話が長いほどこちらが疲れる。',
    '紅衣の監察官': '赤い制服で現れ、校内の小さな違反も見逃さない監察官。規則を守っているか確認するため、まずこちらを殴る。',
    '冬制服の支配者': '季節を無視した冬制服で全員を従わせる支配者。暑さには弱いが、威圧感だけは年中無休。',
    '紫扇の評議員': '扇の一振りで評議会の空気を変える評議員。結論は最初から決まっているのに、会議だけは丁寧に長い。',
    '黒翼の番長': '黒い翼と大げさな二つ名で廊下を制圧する番長。強そうな登場演出のあと、結局は正面から殴ってくる。',
    '新米風紀委員': '校則を覚えたてで、見つけた違反を全部報告したい新米。靴ひもの結び目まで指導対象にする熱心さが怖い。',
    '竹刀の体育係': '号令より先に竹刀が飛んでくる体育係。準備運動は念入りだが、本人だけは最初から本気。',
    '赤ペン監督生': '答案だけでなく人生まで赤ペンで添削する監督生。丸をつけるより、直す場所を探すほうが得意。',
    '図書委員の番人': '静寂を守るためならページをめくる音まで取り締まる番人。本を汚すと怒るが、こちらを殴る音は許容する。',
    '購買部の用心棒': '人気商品を守るため、昼休みの行列を戦場に変えた用心棒。最後の焼きそばパンをめぐる目が本気すぎる。',
    '軽音部の刺客': '音量を情熱と呼び、耳鳴りをアンコールと呼ぶ刺客。演奏が終わるまで攻撃も止まらない。',
    'バスケ部の壁': 'ゴール前に立つだけで作戦の八割を消す壁。突破口はあるらしいが、本人が全部ふさいでいる。',
    '化学部の実験兵': '薬品の扱いより、爆発した後の言い訳がうまい実験兵。「貴重なデータ」で済ませようとしてくる。',
    '新聞部の追跡者': 'スクープのためなら校内の物陰も心の隙も追う記者。取材拒否をすると、拒否した事実まで記事になる。',
    '生徒会の斥候': '生徒会の都合のいい情報だけを集めてくる斥候。見つかった時点で、議題にされることは決まっている。',
    '剣道場の門番': '一礼しない者を通さない門番。礼儀正しく頭を下げても、ついでに一本取ろうとしてくる。',
    '応援団の番長': '応援しているのか威嚇しているのか分からない番長。声援が大きすぎて、敗北の理由まで聞こえない。',
    '保健委員の執行者': '体調不良を見つけると、休ませる前に容赦なく処置する執行者。心配しているのに、なぜかHPが減る。',
    '美術部の幻術師': '「表現」と言い張って、現実の輪郭まで描き変える幻術師。こちらの苦しみも作品名をつければ満足らしい。',
    '吹奏楽部の号令手': '音程より先に号令が届く吹奏楽部員。演奏の合間にも隊列を整えるので、戦闘が行進になる。',
    '放送部の支配者': '校内放送で全員の注意を奪う支配者。マイクを握ると、こちらの反論まで校内全域に生中継される。',
    '進路指導の補佐': '進路希望を聞きながら、すでに不安を答案用紙に書き込む補佐。未来は明るいと言うが、目の前は赤ペンで真っ赤。',
    '数学研究会の刺客': '証明できない攻撃は存在しないと言い張る刺客。倒されても「その反例は想定済み」と言いそうで、素直に負けてくれない。',
    '白手袋の監察員': '埃ひとつを見逃さない白手袋の監察員。潔癖な検査のついでに、こちらの防御の隙まで拭き取ってくる。',
    '文化祭実行委員': '進行表を片手に、祭りも戦闘も予定どおり進めようとする委員。予定外なのは、こちらがまだ倒れていないことだけ。',
    '風紀委員副隊長': '風紀を乱すものを見つける副隊長。自分の威圧感が一番校内の空気を乱していることには気づかない。',
    '剣道部の主将': '一太刀で話を終わらせる剣道部の主将。長い説教より、短い一本のほうが効率的だと知っている。',
    '赤ペン試験官': '答案の余白まで赤線で埋める試験官。正解しても「設問の意図」を説明させるので、勝利にも追試がつく。',
    '禁書庫の司書': '禁書の封印より、貸出手続きを重視する司書。世界が滅びそうでも、返却日と署名欄は見逃さない。',
    '購買部の番人': '売り切れ札を掲げるだけで列全体を絶望させる番人。商品はないのに、戦う体力だけは残っている。',
    '軽音部ギタリスト': 'ギターソロで会話を省略するギタリスト。音が大きいので、本人は説得に成功したつもりでいる。',
    'バスケ部エース': '速攻と自信だけは誰より速いエース。ゴールを外しても「フェイント」と言い張るので、反省という選択肢がない。',
    '化学部の白衣兵': '白衣のポケットから次々と薬品を出す白衣兵。成分表示はないが、危険度だけはよく分かる。',
    '新聞部カメラマン': '決定的瞬間を狙って、戦闘中もシャッターを切り続けるカメラマン。負けた写真まで「貴重な敗北資料」として一面に載せる。',
    '生徒会の策士': '議案と根回しで相手を追い詰める策士。戦う前に勝敗を決めたつもりだが、議事録にはこちらの反論欄がない。',
    '茶道部の令嬢': '一服の作法に厳しい茶道部の令嬢。無礼な攻撃には、静かな所作でこちらのHPだけをきっちり削る。',
    '弓道部の射手': '的を外さない弓道部の射手。狙いは正確だが、なぜ戦闘相手まで的に含めたのかは説明してくれない。',
    '陸上部の疾走者': 'スタートの合図を待たずに走り出す疾走者。速すぎて、攻撃のあとに「今のは準備」と言い張る。',
    '演劇部の仮面役者': '役になりきるあまり、敗北しても別人のふりを続ける役者。台詞は見事だが、脚本にこちらの勝利はなかったことにしたいらしい。',
    '電算部ハッカー': '校内システムも戦闘ログも勝手に書き換えるハッカー。負けそうになると、現実のほうを「仕様変更」しようとする。',
    '天文部の予言者': '星を見て次の一手を予言する天文部員。予言は当たるが、本人が説明している間に星が一周している。',
    '園芸委員の剪定者': '枝だけでなく、こちらの選択肢まで剪定する園芸委員。育てるのは得意だが、敵には雑草扱いである。',
    '試験女王': '試験と採点で校内を支配する女王。正解しても「当然」としか言わず、間違えると嬉しそうに追試を用意する。',
};
...
const getHighSchoolEnemyDescription = (enemy: CompendiumEnemy, displayName: string) => {
    ...
    const characterDescription = HIGH_SCHOOL_CHARACTER_DESCRIPTION_OVERRIDES[displayName];
            ...
            {selectedThemedEnding && (
                <div className="fixed inset-0 z-[75]">
                    <ThemedEndingSequenceScreen
                        ...
                        appearanceMode={selectedThemedEnding.appearanceMode ?? (selectedThemedEnding.variant.pages.some(page => page.imagePath.includes('high-school-vacation')) ? 'VACATION' : 'STANDARD')}
