...
export const createPixelSpriteCanvas = (seed: string, name: string = "", size: number = 16): HTMLCanvasElement => {
    ...
    else if (n.includes('牛乳') || n.includes('ミルク')) spriteKey = 'MILK_CARTON';
    else if (n.includes('焼きそばパン')) spriteKey = 'YAKISOBA_PAN';
    else if (n.includes('クレヨン')) spriteKey = 'CRAYON';
    else if (n.includes('削り器') || n.includes('研')) spriteKey = 'SHARPENER';
    else if (n.includes('鍵')) spriteKey = 'KEY';
    else if (n.includes('名札') || n.includes('校章')) spriteKey = 'SMILE_BADGE';
    else if (n.includes('虫眼鏡')) spriteKey = 'MAGNIFYING_GLASS';
    else if (n.includes('クリスタル') || n.includes('かけら')) spriteKey = 'CRYSTAL_SHARD';
    else if (n.includes('筆箱') || n.includes('鉛筆立て')) spriteKey = 'PENCIL_STAND';
