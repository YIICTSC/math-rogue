      ...
      <div className="z-10 bg-black/80 border-b-4 border-pink-600 p-4 rounded-t-xl shrink-0 flex justify-between items-end">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-pink-200 mb-1 flex items-center gap-2">
            <Info size={24} className="text-pink-500" /> {languageMode === 'ENGLISH' ? 'Lunch Duty Deck Builder' : trans('給食当番の献立作成', languageMode)}
          </h2>
          <p className="text-xs text-gray-400">{languageMode === 'ENGLISH' ? <>Choose <span className="text-pink-400 font-bold">7 cards</span> you have collected to begin your adventure.</> : <>{trans('これまでに獲得したカードから', languageMode)} <span className="text-pink-400 font-bold">7{trans('枚', languageMode)}</span> {trans('選んで冒険を開始します。', languageMode)}</>}</p>
        </div>
      ...
      <div className="z-10 flex-grow overflow-y-auto custom-scrollbar p-4 bg-black/40">
        {selectableCards.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-gray-500 text-center p-8">
            <p className="mb-2 italic text-lg">{languageMode === 'ENGLISH' ? 'No cards are available to select...' : trans('選択できるカードがありません...', languageMode)}</p>
            <p className="text-sm">{languageMode === 'ENGLISH' ? 'Adventure with other characters to unlock cards.' : trans('他のキャラクターで冒険してカードをアンロックしてください。', languageMode)}</p>
          </div>
        ) : (
