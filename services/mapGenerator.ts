...
export const generateDungeonMap = (): MapNode[] => {
  ...
  for (let y = 1; y < MAP_HEIGHT - 1; y++) {
    ...
    sortedPositions.forEach(x => {
        ...
        if (y === 1) {
            ...
            type = NodeType.ELITE; // Guaranteed elite mid-way
            ...
            else if (r < 0.75) type = NodeType.SHOP;
            else if (r < 0.90) type = NodeType.REST;
  ...
  for (let y = 0; y < MAP_HEIGHT - 1; y++) {
      ...
      currentFloor.forEach(curr => {
          ...
          // Strategy: Connect to the absolute closest one always.
          // Then maybe connect to 2nd closest if it is near enough.
          ...
          if (sortedCandidates.length > 0) {
              ...
              // Only if it is reasonably close (within 2 units)
