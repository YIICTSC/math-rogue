...
try {
  ...
  for (const [mode, problems] of Object.entries(SUBJECT_DATA).filter(([mode]) => mode.startsWith('SCIENCE_'))) {
    ...
    for (const problem of visuals) {
      ...
      const direct = collect(problem.visual)
        ...
        .filter((value) => value.length >= 2 && answer.length >= 2 && (value.includes(answer) || answer.includes(value)));
      if (direct.length) {
        hits += 1;
  ...
  console.log(`SCIENCE_VISUAL_UNITS ${units}`);
  console.log(`SCIENCE_DIRECT_ANSWER_HITS ${hits}`);
} finally {
  await server.close();
}
