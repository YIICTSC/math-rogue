...
const collect = (value, out = []) => {
  if (typeof value === 'string') out.push(value);
    ...
    Object.entries(value).forEach(([key, child]) => {
      if (!['kind', 'mode', 'focus', 'tone'].includes(key)) collect(child, out);
    });
  }
  return out;
};
...
try {
  ...
  let units = 0;
  let hits = 0;
  for (const [mode, problems] of Object.entries(SUBJECT_DATA).filter(([mode]) => mode.startsWith('ENGLISH_'))) {
  ...
  console.log(`ENGLISH_VISUAL_UNITS ${units}`);
  console.log(`ENGLISH_DIRECT_ANSWER_HITS ${hits}`);
} finally {
  await server.close();
}
